...
export class GenreController {
    ...
    async list(
        ...
        if (sortField && !GENRE_PAGINATION_ALLOWED_SORT_FIELDS.includes(sortField)) {
            throw new BadRequestException(`Invalid sort field: ${sortField}. Allowed fields are: ${GENRE_PAGINATION_ALLOWED_SORT_FIELDS.join(', ')}`);
        }
        ...
        const genres: GenreEntity[] = await this.genreService.findAllWithBooks(
            find,
            {
                paging: {
                    limit: _limit,
